package at.hannibal2.skyhanni.features.rift.area.lagoon
...
object LagoonCaveParkour {
    ...
    fun onRepoReload(event: RepositoryReloadEvent) {
        val data = event.getConstant<ParkourJson>("parkour/LagoonCaveParkour")
    ...
    fun onChat(event: LorenzChatEvent) {
        ...
        if (event.message == "§cIts toxicity somehow teleported you back!") {
            parkourHelper?.reset()
        }
        if (event.message == "§cIts anger somehow teleported you back!") {
            parkourHelper?.reset()
        }
    }
